...
export function useEventOperations(setEvents: Dispatch<SetStateAction<Event[]>>) {
  ...
  const applyToEvent = async (
    ...
    try {
      ...
      // Send notification but don't await it
 /* sendNotification(eventId, user.id, message, 'application')
    ...
    try {
      ...
      if (status === 'accepted') {
        ...
        if (eventError) throw eventError;
         ...
         // Send acceptance notification but don't await it
        /*sendNotification(eventId, applicantId, '', 'acceptance')
          .catch(err => console.error('Notification error:', err));*/
      }
